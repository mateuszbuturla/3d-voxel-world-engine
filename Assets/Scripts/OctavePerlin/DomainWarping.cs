...
public class DomainWarping : MonoBehaviour
{
    ...
    public float GenerateDomainNoise(int x, int z, NoiseSettings defaultNoiseSettings)
    {
        ...
        return OctavePerlin.Noise(x + domainOffset.x, z + domainOffset.y, defaultNoiseSettings);
    }

    public Vector2 GenerateDomainOffset(int x, int z)
    {
        var noiseX = OctavePerlin.Noise(x, z, noiseDomainX) * amplitudeX;
        var noiseY = OctavePerlin.Noise(x, z, noiseDomainY) * amplitudeY;
